...
class Quaternion extends Array {
  ...
  static slerpUnit (qi, qf, t) {
    ...
    qt[0] = qi[0]*sa + qf[0]*sb
    qt[1] = qi[1]*sa + qf[1]*sb
    qt[2] = qi[2]*sa + qf[2]*sb
    qt[3] = qi[3]*sa + qf[3]*sb
  ...
  static parse (string) {
    try {
      // Extract i,j,k, and real parts from a quaternion string,
      // in any order.
      string = string.replaceAll(' ', '')
      const xst = string.match(/(-?\d*\.?\d+)i/)
      const yst = string.match(/(-?\d*\.?\d+)j/)
      const zst = string.match(/(-?\d*\.?\d+)k/)
      const wst = string.match(/(-?\d*\.?\d+)(?![ijk\.])\b/)
      let x = 0, y = 0, z = 0, w = 0

      // Handle the digitless i, j, k, -i, -j, and -k as special cases.
      if (xst) { x = parseFloat(xst[1]) }
      else {
        if (string.match(/-i/)) { x = -1 }
        else if (string.match(/i/)) { x = 1 }
      }
      if (yst) { y = parseFloat(yst[1]) }
      else {
        if (string.match(/-j/)) { y = -1 }
        else if (string.match(/j/)) { y = 1 }
      }
      if (zst) { z = parseFloat(zst[1]) }
      else {
        if (string.match(/-k/)) { z = -1 }
        else if (string.match(/k/)) { z = 1 }
      }
      if (wst) { w = parseFloat(wst[1]) }
      
      return Quaternion.from([x, y, z, w])

    } catch (e) {
      return Quaternion.from([0, 0, 0, 0])
    }
  }
  ...
  toFixedString (n = 2) {
    return `${    this[0].toFixed(n).padStart(n+3)
          }i + ${ this[1].toFixed(n).padStart(n+3)
          }j + ${ this[2].toFixed(n).padStart(n+3)
          }k + ${ this[3].toFixed(n).padStart(n+3)
          }`
  }
  ...
  conjugate () {
    return Quaternion.from([-this[0], -this[1], -this[2], this[3]])
  }
  ...
  geodesicDistance (q) {
    return Math.acos( 2*this.inner(q)**2 - 1)
  }
  ...
  atAngle (Θ) {
    ...
    if (this[3] !== 1 || (    Math.abs(this[0]) !== 1
                          &&  Math.abs(this[1]) !== 1
                          &&  Math.abs(this[2]) !== 1 )) {
      throw new Error('Quaternion not in correct format for atAngle'
        + ' (expecting 1 ± i, j, or k.)')
    }
