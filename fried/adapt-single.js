...
try {
  ...
  function commonTesseractAnimation () {
    ...
    if (state.checkboxes[1]) {
      state.beta = this.dt / 60
      state.gamma = this.dt / 47
    }
  ...
  function initLofiCube () {
    ...
    const lofiTexture = blankTexture(gl, blurRes, gl.RGBA)
    const depthTexture = blankTexture(gl, blurRes, gl.DEPTH_COMPONENT)
    ...
    this.shared.blurRes = blurRes
    this.shared.fboLofi = fboLofi
    this.shared.lofiTexture = lofiTexture
    this.shared.clearTexture = lofiTexture  // Duplicated for consistency
                                            // with tesseract blur code
  ...
  function initClearTesseract () {
    ...
    this.uMV3 = gl.getUniformLocation(this.program, 'MV3')
    this.uMV4 = gl.getUniformLocation(this.program, 'MV4')
  ...
  function drawClearTesseract () {
    ...
    gl.uniformMatrix4fv(this.uMV3, false, this.MV3)
    gl.uniformMatrix4fv(this.uMV4, false, this.MV4)

    gl.bindFramebuffer(gl.FRAMEBUFFER, this.shared.fboClear)
    gl.viewport(0, 0, this.shared.res, this.shared.res)
    gl.clearColor(0, 0, 0, 0) // zero α for continuity in blur effect
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT)
  ...
  function drawTesseractCompositor () {
    ...
    if (state.checkboxes[2]) {
      wd = Math.sin(τ * (this.dt / 6000)) * 0.5 + 0.9
    }
  ...
  state.animation1.draw = glPipeline(gl, { animationState: state.animation1 },
  [
    {
      vertexShader: shaders.projector4dVert,
      fragmentShader: shaders.greenFromWFrag,
      mesh: geometry.tesseract,
      components: 4,
      init: initClearTesseract,
      draw: drawClearTesseract
    },
  ...
  // state.animation2.draw = glPipeline(gl2, { animationState: state.animation2 },
  // [
  //   {
  //     vertexShader: shaders.projector4dVert,
  //     fragmentShader: shaders.greenFromWFrag,
  ...
  /*[
    ...
    { // draw the scene clearly, at full resolution:
      vertexShader: shaders.redNormalV,
      fragmentShader: shaders.plainColorF,
    ...
    // },
    // { // post-process the output with iterated Gaussian blur:
    //   vertexShader: shaders.textureV,
    //   fragmentShader: shaders.blur1dF,
    //   mesh: geometry.texSquare,
    //   init: initBlur,
    //   draw: drawBlur
    // },
    ...
    //   vertexShader: shaders.textureV,
    //   fragmentShader: shaders.blurCompositorF,
    //   mesh: geometry.texSquare,
 ...
 * @param {WebGLRenderingContext} context The context binding this texture
 ...
 * @param {WebGLRenderingContext} gl WebGL context to use
 */
function verifyFramebuffer(gl) {
  const status = gl.checkFramebufferStatus(gl.FRAMEBUFFER)
  if (status !== gl.FRAMEBUFFER_COMPLETE) {
    logError('Framebuffer incomplete: '
      + Object.entries(gl).find(x => x[1] === status)[0])
  }
}
...
function initListeners () {
  ...
  window.addEventListener('resize', event => {
    ...
    for (const s of [state.animation1]) {
      const ctx = s.context
      ctx.canvas.width = ctx.canvas.clientWidth
      ctx.canvas.height = ctx.canvas.clientHeight
