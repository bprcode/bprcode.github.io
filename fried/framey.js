...
try {
  ...
  const texQuad = Mesh.from(flatQuad)
    texQuad.stride = 3
    texQuad.interleave(v => [.5*v[0]+.5, .5*v[1]+.5]) // Generate tex coords
  ...
  // Initialization callback for framebuffer demo
  function initFramey () {
    /** @type {WebGLRenderingContext} */
    const gl = this.gl
    this.fbSize = 256

    // Acquire a texture name, allocate space for a size x size RGBA texture
    this.animationState.textureTarget = gl.createTexture()
    gl.bindTexture(gl.TEXTURE_2D, this.animationState.textureTarget)
    gl.texImage2D(gl.TEXTURE_2D,
      ...
      gl.UNSIGNED_BYTE, // 8 bits per channel
      null // no initial upload
      )
    ...
    // Create a framebuffer object, bind its color attachment
    this.animationState.fbo = gl.createFramebuffer()
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.animationState.fbo)
    
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0,
      gl.TEXTURE_2D, this.animationState.textureTarget, 0)

    // Also attach a depth renderBuffer
    ...
    // If the framebuffer was not completely initialized, report the error.
    const framebufferStatus = gl.checkFramebufferStatus(gl.FRAMEBUFFER)
    if (framebufferStatus !== gl.FRAMEBUFFER_COMPLETE) {
      logError('Framebuffer initialization incomplete: '
  ...
  // Per-frame operations for framebuffer demo
  function stageFramey () {
    /** @type {WebGLRenderingContext} */
    const gl = this.gl
    ...
    const mirror = [
      ...
      0, -1, 0, 0,
      0, 0, 1, 0,
      0, -2*mirrorHeight, 0, 1
    ]
    orbiter.call(this)

    const previousMatrix = [...this.MV]
    gl.frontFace(gl.CW)
    // mult4(this.MV, mirror, this.MV)

    mult4(this.R, mirror, this.R)
    gl.uniformMatrix4fv(this.reflection, false, this.R)

    // Draw to the off-screen framebuffer
    gl.viewport(0, 0, this.fbSize, this.fbSize)
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.animationState.fbo)
    const transparentClear = this.clearColor
    transparentClear[3] = 0
    gl.clearColor(...transparentClear)
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT)
    gl.clearColor(...this.clearColor)

    gl.uniformMatrix4fv(this.projection, false, this.P)
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER, null)
    this.MV = [...previousMatrix]
    ident(this.R)

    // gl.bindVertexArray(null)
    // gl.bindBuffer(gl.ARRAY_BUFFER, this.vbo)
    // gl.enableVertexAttribArray(this.pos)
    // gl.vertexAttribPointer(this.pos, 3, gl.FLOAT, false,
    //   this.mesh.byteStride, 0)

    // gl.drawArrays(gl.LINE_STRIP, 0, this.mesh.blocks)

    // gl.enableVertexAttribArray(this.normal)
    // gl.vertexAttribPointer(this.normal, 3, gl.FLOAT, false,
    //   this.mesh.byteStride, 3 * Float32Array.BYTES_PER_ELEMENT)
    // return
  }

  function stageFrameyTexturer () {
    /** @type {WebGLRenderingContext} */
    const gl = this.gl
    gl.bindTexture(gl.TEXTURE_2D, this.animationState.textureTarget)
    gl.uniform1fv(this.uKernel, state.kernel)

    // mult4(this.MV, translateMatrix(0,0,-1), this.MV)
    // mult4(this.MV, scaleMatrix(9), this.MV) // debug there is a better way
    // mult4(this.MV, translateMatrix(0,0,-40), this.MV)
    const previousMV = [...this.MV]
    const previousP = [...this.P]
    ident(this.MV)
    ident(this.P)
    mult4(this.MV, translateMatrix(0,0,-1), this.MV)
    ...
    this.P.length = 0
    this.P.push(...previousP)
    this.MV.length = 0
    this.MV.push(...previousMV)
  }

  function initFrameyTexturer () {
    const gl = this.gl
    ...
    const texSize = 64
    gl.uniform1f(this.uSize, texSize)
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA,
      texSize, texSize, 0,
      gl.RGBA, gl.UNSIGNED_BYTE, checkerboard())

    function checkerboard () {
      const board = new Uint8Array(texSize * texSize * 4)
      const fraction = texSize / 4
      for (let i = 0; i < texSize; i++){
        const g = Math.floor(2 * (i % fraction) / fraction)
        for (let j = 0; j < texSize; j++) {
          const f = Math.floor(2 * (j % fraction) / fraction)
          board.set([255 * f, 64 * g, 100, 64],
            (i * texSize + j) * 4)
        }
      }
      return board
    ...
    function loadLocalImage () {
      ...
      image.onload = () => {
        ...
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA,gl.RGBA,
          ...
          logError(`Unable to access ${image.src}\n`
          + e.message + '\n')
        }
      }
...
try {
  ...
  for (const p of phases) {
    ...
    p.gl = gl
    p.animationState ??= props.animationState
    p.clearColor ??= props.clearColor || [0.1, 0.1, 0.1, 1]
    p.P = P // Shared reference to projection matrix
    ...
    p.reflection = gl.getUniformLocation(p.program, 'reflection')
