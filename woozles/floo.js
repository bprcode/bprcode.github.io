...
function honeycomb (gl, props = {}) {
  ...
  const vs = compileShader(gl, vertexShader2d, gl.VERTEX_SHADER)
  const fs = compileShader(gl, flatFrag, gl.FRAGMENT_SHADER)
  const program = gl.createProgram()
  gl.attachShader(program, vs)
  gl.attachShader(program, fs)
  linkProgram(gl, program)
  ...
  if (props.ab === 'A') {
    ...
    show(M, 'Initial honeycomb matrix A')
  } else {
    M = product4x4(rotateZMatrix(π / 4), M)
    M = product4x4(scaleMatrix(0.5), M)
    M = product4x4(translateMatrix(0.3,0.1,0), M)
    show(M, 'Initial honeycomb matrix B')
  }

  gl.enable(gl.CULL_FACE)
  gl.useProgram(program)
  const clearColor = props.clearColor || [0.15, 0.15, 0.15, 1]
  gl.clearColor(...clearColor)
...
void main(void) {
  finalColor = vec4(0.4 + 0.4*cos(10.0*6.28*t),
                    pow(distance(center, newPos) / size, 2.5),
                    0.5,
                    1.0);
}`
...
honeycomb(gl, { clearColor: [0.15, 0.1, 0.25, 1], ab: 'A' })
honeycomb(gl2)
