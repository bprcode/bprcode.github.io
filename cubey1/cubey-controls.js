...
try {
  ...
  log('tessearct byteStride is ', tesseract.byteStride)
  ...
  document.getElementById('yz-slider').value = 1
  document.getElementById('xz-slider').value = 1
  document.getElementById('yz-slider')
...
try {
  ...
  for (const p of phases) {
    ...
    p.u_MV3 = gl.getUniformLocation(p.program, 'MV3')
    p.u_MV4 = gl.getUniformLocation(p.program, 'MV4')
  ...
  function drawFrame(t) {
    ...
    for (const p of phases) {
      ...
      gl.drawArrays(p.drawMode ?? gl.TRIANGLES, 0,
                      p.mesh?.blocks ?? square2d.length / 2)
    }
